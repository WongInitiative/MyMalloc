...
#define _MYMALLOC_H
	...
	#define free(x) myfree((x), __LINE__, __FILE__)
        ...
        typedef struct meta{ //took a decent BIT of effort figuring out how to make this smaller (can I have more points for the pun please I'm desperate)
                unsigned int isfreeNsize : 14; //creates bitfield of exactly 14 bits. 13 bits for the size of the usable space(which must be less than 4096), 1 bit leftover to represent if block is free or not
